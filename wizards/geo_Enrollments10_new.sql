...
create procedure <?= DB_SCHEMA ?>.geo_Enrollments10_new(
...
begin
	...
	where e._school = __schoolid and e._enrollmentmode = 'CONED' and e._year = 2022 and e._amount = __coned;
	if __coned > 0 and __enrollexists = -1 then
		insert into <?= DB_SCHEMA ?>.geo_enrollments (_school, _enrollmentmode, _year, _amount) values
		(__schoolid, 'CONED', 2022, __coned);
	end if;
end//

delimiter //

create procedure <?= DB_SCHEMA ?>.dev_Enrollment_Fix(
	in __id int,
	in __school int,
	in __enrollmentmode Varchar(8),
	in __year Year,
	in __amount int)
begin
	insert into <?= DB_SCHEMA ?>.geo_enrollments (_school, _enrollmentmode, _year, _amount)
	values (__school, __enrollmentmode, __year, __amount);
end//
