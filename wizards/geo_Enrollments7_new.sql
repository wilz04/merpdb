...
create procedure <?= DB_SCHEMA ?>.geo_Enrollments7_new(
...
begin
	...
	where e._school = __schoolid and e._enrollmentmode = 'EN' and e._year = 2022 and e._amount = __en;
	if __en > 0 and __enrollexists = -1 then
		insert into <?= DB_SCHEMA ?>.geo_enrollments (_school, _enrollmentmode, _year, _amount) values
		(__schoolid, 'EN', 2022, __en);
	end if;
end//
