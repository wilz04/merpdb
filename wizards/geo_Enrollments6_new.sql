...
create procedure <?= DB_SCHEMA ?>.geo_Enrollments6_new(
...
begin
	...
	where e._school = __schoolid and e._enrollmentmode = 'CAIPAD' and e._year = 2022 and e._amount = __caipad;
	if __caipad > 0 and __enrollexists = -1 then
		insert into <?= DB_SCHEMA ?>.geo_enrollments (_school, _enrollmentmode, _year, _amount) values
		(__schoolid, 'CAIPAD', 2022, __caipad);
	end if;
end//
