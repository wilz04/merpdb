...
create procedure <?= DB_SCHEMA ?>.geo_Enrollments4_new(
...
begin
	...
	where e._school = __schoolid and e._enrollmentmode = 'CNVMTS' and e._year = 2022 and e._amount = __cnvmts;
	if __cnvmts > 0 and __enrollexists = -1 then
		insert into <?= DB_SCHEMA ?>.geo_enrollments (_school, _enrollmentmode, _year, _amount) values
		(__schoolid, 'CNVMTS', 2022, __cnvmts);
	end if;
end//
