create view <?= DB_SCHEMA ?>.geo_enrollments2_list as select
...
left join <?= DB_SCHEMA ?>.geo_enrollments ad on ad._enrollmentmode = 'AD' and ad._school = s._id and ad._year = y._id
left join <?= DB_SCHEMA ?>.geo_enrollments ae on ae._enrollmentmode = 'AE' and ae._school = s._id and ae._year = y._id
left join <?= DB_SCHEMA ?>.geo_enrollments pr on pr._enrollmentmode = 'PR' and pr._school = s._id and pr._year = y._id
left join <?= DB_SCHEMA ?>.geo_enrollments rc on rc._enrollmentmode = 'RC' and rc._school = s._id and rc._year = y._id
left join <?= DB_SCHEMA ?>.geo_enrollments pe on pe._enrollmentmode = 'PE' and pe._school = s._id and pe._year = y._id
left join <?= DB_SCHEMA ?>.geo_enrollments pea on pea._enrollmentmode = 'PEA' and pea._school = s._id and pea._year = y._id
where
	ifNull(ad._amount, 0) > 0 or
	ifNull(ae._amount, 0) > 0 or
	ifNull(pr._amount, 0) > 0 or
	ifNull(rc._amount, 0) > 0 or
	ifNull(pe._amount, 0) > 0 or
